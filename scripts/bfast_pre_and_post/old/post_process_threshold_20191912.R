...
system(sprintf("gdalbuildvrt %s %s",
               paste0(res_dir,"results_tshuapa.vrt"),
...
system(sprintf("gdal_translate -co COMPRESS=LZW %s %s",
               paste0(res_dir,"results_tshuapa.vrt"),
               paste0(res_dir,"results_tshuapa.tif")
))
...
stats <- paste0(res_dir,"stats_tshuapa.txt")
...
system(sprintf("gdal_translate -ot Byte -co COMPRESS=LZW %s %s",
               paste0(res_dir,"tmp_results_magnitude_pct.tif"),
               paste0(res_dir,"results_tshuapa_threshold.tif")
))
...
#################### CANEVAS
mask   <- paste0(res_dir,"results_tshuapa_threshold.tif")

system(sprintf("ogr2ogr -t_srs EPSG:4326 %s %s",
               paste0(res_dir,"geo_tshuapa.shp"),
               paste0(res_dir,"tshuapa.shp")))
...
system(sprintf("python %s -v %s -i %s -o %s -a %s",
               ...
               paste0(res_dir,"geo_tshuapa.shp"),
               paste0(res_dir,"results_tshuapa_threshold.tif"),
               paste0(res_dir,"limites_tshuapa.tif"),
               "CODE_INS"
))
...
#################### ALIGN
thresh  <- paste0(res_dir,"results_tshuapa_threshold.tif")
lc_map  <- paste0(res_dir,"aligned_masque_NF_F_Def_2014_2016_diaf_jica.tif")
limites <- paste0(res_dir,"limites_tshuapa.tif")
...
system(sprintf("gdal_translate -ot Byte -co COMPRESS=LZW %s %s",
               paste0(res_dir,"tmp_map_bfast_pct.tif"),
               paste0(res_dir,"results_tshuapa_map.tif")
))
