res_dir <- "/home/diaf/bfast_prepost/data/res_2016_2018/nord_ubangui/"
setwd(res_dir)
...
system(sprintf("gdalbuildvrt %s %s",
               paste0(res_dir,"results_nord_ubangi.vrt"),
...
system(sprintf("gdal_translate -co COMPRESS=LZW %s %s",
               paste0(res_dir,"results_nord_ubangi.vrt"),
               paste0(res_dir,"results_nord_ubangi.tif")
))
...
stats <- paste0(res_dir,"stats_nord_ubangi.txt")
...
system(sprintf("gdal_translate -ot Byte -co COMPRESS=LZW %s %s",
               paste0(res_dir,"tmp_results_magnitude_pct.tif"),
               paste0(res_dir,"results_nord_ubangi_threshold.tif")
))
...
#################### CANEVAS
mask   <- paste0(res_dir,"results_nord_ubangi_threshold.tif")

system(sprintf("ogr2ogr -t_srs EPSG:4326 %s %s",
               paste0(res_dir,"geo_nord_ubangi.shp"),
               paste0(res_dir,"limites_nord_ubangi.shp")))

#################### RASTERIZE
system(sprintf("oft-rasterize_attr.py  -v %s -i %s -o %s -a %s",
               paste0(res_dir,"geo_nord_ubangi.shp"),
               paste0(res_dir,"results_nord_ubangi_threshold.tif"),
               paste0(res_dir,"limites_nord_ubangi.tif"),
               "CODE_INS"
))
...
#################### ALIGN
thresh  <- paste0(res_dir,"results_nord_ubangi_threshold.tif")
lc_map  <- paste0(res_dir,"aligned_masque_NF_F_Def_2014_2016_diaf_jica.tif")
limites <- paste0(res_dir,"limites_nord_ubangi.tif")
...
system(sprintf("gdal_translate -ot Byte -co COMPRESS=LZW %s %s",
               paste0(res_dir,"tmp_map_bfast_pct.tif"),
               paste0(res_dir,"results_nord_ubangi_map.tif")
))
