res_dir <- "/home/diaf/bfast_prepost/data/res_2016_2018/"
setwd(res_dir)
...
list_res <- list.files(res_dir,pattern = glob2rx(paste0("final_results*.tif")),recursive = T)

####################  CREATE A VRT OUTPUT
system(sprintf("gdalbuildvrt %s %s",
               paste0(res_dir,"results_rdc.vrt"),
               paste0(res_dir,list_res,collapse=' ')
))


## Compress final result
system(sprintf("gdal_translate -co COMPRESS=LZW -co BIGTIFF=YES %s %s",
               paste0(res_dir,"results_rdc.vrt"),
               paste0(res_dir,"results_rdc.tif")
))

####################  EXTRACT MAGNITUDE
# system(sprintf("gdal_translate -b 2 %s %s", 
#                paste0(res_dir,"results_equateur.tif"),
...
stats <- paste0(res_dir,"stats_rdc.txt")
...
system(sprintf("gdal_translate -ot Byte -co COMPRESS=LZW %s %s",
               paste0(res_dir,"tmp_results_magnitude_pct.tif"),
               paste0(res_dir,"results_rdc_2016_threshold_20210603.tif")
))
...
#################### CANEVAS
mask   <- thresh <- paste0(res_dir,"results_rdc_2016_threshold_20210603.tif")

system(sprintf("ogr2ogr -t_srs EPSG:4326 %s %s",
               paste0(res_dir,"geo_rdc.shp"),
               paste0(res_dir,"RDC_Province_26.shp")))

#################### RASTERIZE
system(sprintf("oft-rasterize_attr.py  -v %s -i %s -o %s -a %s",
               paste0(res_dir,"geo_rdc.shp"),
               thresh,
               paste0(res_dir,"limites_rdc.tif"),
               "CODE_INS"
))

#################### ALIGN
input  <- paste0(res_dir,"masque_NF_F_Def_2014_2016_diaf_jica.tif")
output  <- paste0(res_dir,"aligned_masque_NF_F_Def_2014_2016_diaf_jica.tif")

align(mask, input, output)

#################### ALIGN
lc_map  <- paste0(res_dir,"aligned_masque_NF_F_Def_2014_2016_diaf_jica.tif")
limites <- paste0(res_dir,"limites_rdc.tif")
...
system(sprintf("gdal_translate -ot Byte -co COMPRESS=LZW %s %s",
               paste0(res_dir,"tmp_map_bfast_pct.tif"),
               paste0(res_dir,"results_rdc_annee_2016.tif")
))
