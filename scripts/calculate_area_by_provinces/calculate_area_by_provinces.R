...
system(sprintf("oft-rasterize_attr.py -v %s -i %s -o %s  -a %s",
               ...
               paste0(my_dir,"data/cartes/dd_00_10_14_16_18_v20210504.tif"),
...
system(sprintf("gdal_calc.py -A %s -B %s --type=Int16 --co=\"COMPRESS=LZW\" --outfile=%s --calc=\"%s\"",
               paste0(my_dir,"rdc_provinces_16b.tif"),
               paste0(my_dir,"diaf_00_18_16b.tif"),
               paste0(my_dir,"diaf_00_18_provinces.tif"),
               "A*100+B"
))
...
##################### EXTRAIRE LA RESOLUTION
  pix <- res(raster(paste0(my_dir,"diaf_00_18_provinces.tif")))[1]

##################### LIRE LA TABLE ET CALCULER LES SUPERFICIES
df <- read.table(paste0(my_dir,"stats_00_18.txt"))[,1:2]
...
##################### EXPORTER LE FICHIER
write.csv(df2,paste0(my_dir,"areas_00_18_v20210504.csv"),row.names = F)


df3 <- data.frame(cbind(1:8,tapply(df2$area_ha,df2$change,sum)))
names(df3) <- c("class","area")
write.csv(df3,paste0(my_dir,"areas_00_18_national_v20210504.csv"),row.names = F)
