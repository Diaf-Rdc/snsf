...
##################### RASTERISER LE SHAPEFILE DES PROVINCES SUR LA CARTE 00_16
system(sprintf("oft-rasterize_attr.py -v %s -i %s -o %s  -a %s",
               paste0(my_dir,"data/shp_drc/RDC_Province_26.shp"),
               paste0(my_dir,"data/cartes/results_rdc_annee_2016.tif"),
...
##################### PASSER LA CARTE 00-16 16Bit
system(sprintf("gdal_translate -ot Int16 -co COMPRESS=LZW  %s %s",
               paste0(my_dir,"data/cartes/results_rdc_annee_2016.tif"),
               paste0(my_dir,"diaf_2016_16b.tif")
))
...
system(sprintf("gdal_calc.py -A %s -B %s --type=Int16 --co=\"COMPRESS=LZW\" --outfile=%s --calc=\"%s\"",
               paste0(my_dir,"rdc_provinces_16b.tif"),
               paste0(my_dir,"diaf_2016_16b.tif"),
               paste0(my_dir,"diaf_2016_provinces.tif"),
               "A*100+B"
))
...
classes <- as.numeric(as.vector(outer(1:26,c(2,12,22,40,50),paste, sep="")))
...
##################### EXTRAIRE LA RESOLUTION
  pix <- res(raster(paste0(my_dir,"diaf_2016_provinces.tif")))[1]

##################### LIRE LA TABLE ET CALCULER LES SUPERFICIES
df <- read.table(paste0(my_dir,"stats_2016.txt"))[,1:2]
...
##################### EXPORTER LE FICHIER
write.csv(df2,paste0(my_dir,"areas_2016_v20210604.csv"),row.names = F)


df3 <- data.frame(cbind(1:8,tapply(df2$area_ha,df2$change,sum)))
names(df3) <- c("class","area")
write.csv(df3,paste0(my_dir,"areas_2016_national_v20210604.csv"),row.names = F)
