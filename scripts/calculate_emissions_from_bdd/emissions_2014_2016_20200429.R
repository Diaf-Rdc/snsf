...
bt_foret <- data.frame(readxl::read_xlsx(paste0(datadir,"ef_20200423.xlsx"),"bt_forest"))
bt_nofor <- data.frame(readxl::read_xlsx(paste0(datadir,"ef_20200423.xlsx"),"bt_nonforest"))

##### LIRE LA BDD CONTENANT TOUS LES POINTS INTERPRETÉS
df0 <- read.csv2(paste0(sim1416dir,"sim_bd/bd_simulation_5_2014_2016_20200403.csv"))

##### LIRE LE FICHIER DES SUPERFICIES DE LA CARTE
ar <- read.csv2(paste0(areadir,"comptage_pixel_map_14_16_inputs_sepal_15102019_calcul_emission.csv"))
ar$cls_cl <- paste0(ar$province,ar$class1416)
...
table(df0$quarternary_land_cover_label_filled,df0$ce_change)
...
nrow(df0[(df0$quarternary_land_cover_label_filled == "" | is.na(df0$quarternary_land_cover_label_filled)) & df0$ce_change==3,]) == 0
...
write.csv(df0,paste0(bdd_dir,"bd_filled_simulation_5_2014_2016_20200429.csv"))
...
for(province in unique(ar$NOM)){
  ...
  prop1416 <- as.data.frame.matrix(subc/sum(subc))
  loss1416 <- as.data.frame.matrix(subc/sum(subc)*s[3,"strRS_area_estimate"])       # Comme on a déjà restreint la BD, il n'y a que 3 lignes
  ci1416   <- as.data.frame.matrix(subc/sum(subc)*s[3,"strRS_confidence_interval"]) # Comme on a déjà restreint la BD, il n'y a que 3 lignes
  
  prov1416       <- cbind(row.names(loss1416),prop1416,loss1416,ci1416)
  names(prov1416) <- c("cl_dep",
                       paste0("prop_",names(loss1416)),
                       paste0("area_",names(loss1416)),
                       paste0("ci_area_",names(ci1416)))
  ...
  d1[,paste0("emis_",   names(loss1416))]  <- lapply(names(loss1416),calc_emission)
  d1[,paste0("ci_emis_",names(loss1416))]  <- lapply(names(loss1416),calc_ci_em)
  
  names(d1)
  dd[nrow(dd)+1,"province"] <- province
  
  for(cl_dep in d1$cl_dep){
    for(cl_arr in names(loss1416)){
  ...
  write.csv(d1,paste0(sim1014dir,"sim_em/em_",province,"_1416.csv"),row.names = F)
...
write.csv2(dd,paste0(sim1014dir,"emissions_renf_2014_2016_20200429.csv"),row.names = F)
