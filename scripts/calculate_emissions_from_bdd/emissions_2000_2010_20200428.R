...
df0 <- read.csv2(paste0(sim0010dir,"sim_bd/bd_simulation_14_2000_2010_20200403.csv"))

##### LIRE LE FICHIER DES SUPERFICIES DE LA CARTE
ar <- read.csv(paste0(areadir,"areas_transitions_2000_2010_2014_20190724.csv"))
...
write.csv(df0,paste0(bdd_dir,"bd_filled_simulation_14_2000_2010_20200423.csv"))
...
for(province in unique(ar$NOM)){
  ...
  names(prov0010) <- c("cl_dep",
                       ...
                       paste0("ci_area_",names(ci0010)))
  
  d1 <- merge(prov0010,bt_foret[,c("Class","BT")],by.x="cl_dep",by.y="Class",all.x=T)
  ...
  d1[,paste0("emis_",   names(loss0010))]  <- lapply(names(loss0010),calc_emission)
  d1[,paste0("ci_emis_",names(loss0010))]  <- lapply(names(loss0010),calc_ci_em)
  
  names(d1)
  dd[nrow(dd)+1,"province"] <- province
  
  for(cl_dep in d1$cl_dep){
    for(cl_arr in names(loss0010)){
  ...
  write.csv(d1,paste0(sim0010dir,"sim_em/em_",province,"_0010.csv"),row.names = F)
...
write.csv2(dd,paste0(sim0010dir,"emissions_renf_2000_2010_20200428.csv"),row.names = F)
