...
##### LIRE LA BDD
df <- read.csv2(paste0(sim1014dir,"sim_bd/bd_simulation_28_2010_2014_20200403.csv"))
...
codes <- read.dbf(paste0(datadir,"shp_drc/RDC_Province_26.dbf"))[,c("NOM","ID_SEPAL")]
names(codes) <- c("province_name","ID_SEPAL")

s1  <- merge(s,codes,by.y="ID_SEPAL",by.x="province",all.x=T)

write.csv2(s1,paste0(datadir,"verif_res_sim",simulation,"_2010_2014_20200429.csv"),row.names = F)  
